...
export default function App() {
  ...
  // Load audio tracks whenever the selected book changes.
  useEffect(() => {
    if (!selectedId) { setTracks([]); return; }
    window.bookray.audioGetTracks(selectedId).then(setTracks);
  ...
  function handleDragOver(e: React.DragEvent) {
    e.preventDefault();
    if ([...e.dataTransfer.items].some((item) => item.kind === 'file')) setIsDragging(true);
  }
  ...
  return (
    <div
      className="flex flex-col h-screen bg-slate-950 text-white overflow-hidden relative"
    ...
    >
        ...
        <div className="absolute inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center pointer-events-none">
          <div className="border-2 border-dashed border-blue-500 rounded-2xl px-16 py-12 flex flex-col items-center gap-3">
            <svg viewBox="0 0 24 24" fill="none" className="w-10 h-10 text-blue-400">
              ...
              <path d="M4 16.5A3.5 3.5 0 0 0 7.5 20h9a3.5 3.5 0 0 0 3.5-3.5c0-1.63-1.1-3-2.6-3.4" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" />
            </svg>
            <p className="text-blue-400 font-medium text-sm">Drop EPUB to open</p>
          </div>
        </div>
      )}

      {/* Main content row — takes all space above the player bar */}
      ...
      {/* Audio player bar — only rendered when tracks are loaded */}
