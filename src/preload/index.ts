...
const AUDIO_GET_TRACKS      = 'audio:get-tracks';
const AUDIO_IMPORT_FOLDER   = 'audio:import-folder';
const AUDIO_UPDATE_DURATION = 'audio:update-duration';
...
contextBridge.exposeInMainWorld('bookray', {
  ...
  audioGetTracks:      (bookId: string)                        => ipcRenderer.invoke(AUDIO_GET_TRACKS, bookId),
  audioImportFolder:   (bookId: string)                        => ipcRenderer.invoke(AUDIO_IMPORT_FOLDER, bookId),
  audioUpdateDuration: (trackId: string, duration: number)     => ipcRenderer.invoke(AUDIO_UPDATE_DURATION, trackId, duration),
